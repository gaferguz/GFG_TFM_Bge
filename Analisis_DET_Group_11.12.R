...
contador = 0
lev2 <- levels(as.factor(samples[,3])) # get the variables
...
for (j in 1:length(L.pairs)) {
    ...
    factor1vs2 <- results(dds_all, contrast=c("group", factor1, factor2), alpha = 0.05)
    factor1vs2 <- lfcShrink(dds_all, contrast=c("group", factor1, factor2), res=factor1vs2, type = "ashr") #type = "normal") # Este tipo funciona
...
Groups <- grid.arrange(DESeq2_Ctrl_vs_GFa, DESeq2_Ctrl_vs_GFaFeces, DESeq2_GFa_vs_GFaFeces,
                       nrow=1, ncol=3)
...
for(i in 1:length(deseq2_plots)){ 
  ...
  #save the enrichment result
  write.csv(file = paste0(title,"_under_ORA.csv"), x = under_enrichment@result)
