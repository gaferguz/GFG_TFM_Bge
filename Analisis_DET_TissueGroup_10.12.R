...
dds <- DESeq(dds)
vsd <- vst(dds)
PCAtissue <- plotPCA3D(vsd, intgroup = "tissue", ntop = 61092)
PCAcond <- plotPCA3D(vsd, intgroup = "condition")
PCAgroup = plotPCA3D(vsd, intgroup = "group")
...
# Buscamos la equivalencia en PCA filtrando los 500
...
for (l in lev) {
  ...
  txi.kallisto <- tximport(subfiles, type = "kallisto", txOut = TRUE) # Esta da error
  ...
  for (j in 1:length(L.pairs)) {
    ...
    factor1vs2 <- lfcShrink(dds_all, contrast=c("condition", factor1, factor2), res=factor1vs2, type = "ashr") #type = "normal") # Este tipo funciona
...
for(i in 1:length(deseq2_plots)){ 
  ...
  subtitle_plot <- paste0("DETs: ", nrow(reportf))
  selected_tx <- c()
...
FB <- grid.arrange(DESeq2_FB_Ctrl_vs_FB_G2r_plot, 
                   DESeq2_FB_Ctrl_vs_FB_GFa_plot,
                   DESeq2_FB_G2r_vs_FB_GFa_plot,
                   nrow=1,
                   ncol=3)

FG <- grid.arrange(DESeq2_FG_Ctrl_vs_FG_GFa_plot, 
                   DESeq2_FG_Ctrl_vs_FG_GFaFeces_plot,
                   DESeq2_FG_GFa_vs_FG_GFaFeces_plot,
                   nrow=1,
                   ncol=3)

MG <- grid.arrange(DESeq2_MG_Ctrl_vs_MG_GFa_plot, 
                   DESeq2_MG_Ctrl_vs_MG_GFaFeces_plot,
                   DESeq2_MG_GFa_vs_MG_GFaFeces_plot,
                   nrow=1,
                   ncol=3)

HG <- grid.arrange(DESeq2_HG_Ctrl_vs_HG_GFa_plot, 
                   DESeq2_HG_Ctrl_vs_HG_GFaFeces_plot,
                   DESeq2_HG_GFa_vs_HG_GFaFeces_plot,
                   nrow=1,
                   ncol=3)

MT <- grid.arrange(DESeq2_MT_Ctrl_vs_MT_GFa_plot, 
                   DESeq2_MT_Ctrl_vs_MT_GFaFeces_plot,
                   DESeq2_MT_GFa_vs_MT_GFaFeces_plot,
                   nrow=1,
                   ncol=3)

SG <- grid.arrange(DESeq2_SG_Ctrl_vs_SG_GFa_plot, 
                   DESeq2_SG_Ctrl_vs_SG_GFaFeces_plot,
                   DESeq2_SG_GFa_vs_SG_GFaFeces_plot,
                   nrow=1,
                   ncol=3)
...
for (i in 1:length(deseq2_plots)) {
  ...
  ggsave(x, file = paste0(path,"/", deseq2_plots[[i]], ".png"), width = 9, height = 9, dpi = 300)
}
